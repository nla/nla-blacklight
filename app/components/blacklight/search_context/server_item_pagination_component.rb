...
# Override of Blacklight::SearchContext::ServerItemPaginationComponent to fix
# search context pagination not rendering after Blacklight 9.0.0 upgrade.
# The gem's render? guard requires @search_session['document_id'] to match
# the current document, but this value can be stale or unset in certain
# navigation flows, causing the pagination component to disappear.
# See: https://github.com/projectblacklight/blacklight/blob/v9.0.0/app/components/blacklight/search_context/server_item_pagination_component.rb
module Blacklight
  module SearchContext
    class ServerItemPaginationComponent < Blacklight::Component
      ...
      def render?
        @search_context.present? && (@search_context[:prev] || @search_context[:next] || total.positive?)
      end
