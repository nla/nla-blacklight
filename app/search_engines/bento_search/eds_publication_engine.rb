...
class BentoSearch::EdsPublicationEngine
  ...
  AUTH_HEADER = "x-authenticationToken"
  SESSION_TOKEN_HEADER = "x-sessionToken"
      ...
      with_session(end_user_auth) do |session_token|
        ...
        response.xpath("./PublicationSearchResponseMessageGet/SearchResult/Data/Records/Record").each do |record_xml|
          ...
          if item.authors.blank?
            ...
            search_links = author_xml.xpath(".//searchLink")
            if search_links.size > 0
          ...
          record_xml.xpath("./Items/Item[child::Group[text()='URL']]").each do |url_item|
            ...
            if data_element.text.strip.start_with?("<link")
              ...
              next if data_element.text.strip.blank?
  ...
  def with_session(auth = false, &block)
    ...
    unless response_xml && (session_token = at_xpath_text(response_xml, "//SessionToken"))
      EdsCommException.new("Could not get SessionToken")
    end
  ...
  def get_with_auth(url, session_token = nil)
    ...
    begin
      headers = {AUTH_HEADER => auth_token, "Accept" => "application/xml"}
      headers[SESSION_TOKEN_HEADER] = session_token if session_token

      s_time = Time.zone.now
      response = http_client.get(url, nil, headers)
      Rails.logger.debug { "EDS timing GET: #{Time.zone.now - s_time}:#{url}" }
      ...
      if (at_xpath_text(response_xml, "//ErrorNumber") == "104") || (at_xpath_text(response_xml, "//ErrorDescription") == "Auth Token Invalid")
        ...
        headers[AUTH_HEADER] = self.class.remembered_auth = get_auth_token
        response = http_client.get(url, nil, headers)
        response_xml = Nokogiri::XML(response.body)
        response_xml.remove_namespaces!
      end
    ...
    if response.nil? || response_xml.nil? || caught_exception || (!HTTP::Status.successful? response.status)
      exception = EdsCommException.new("Error fetching URL: #{caught_exception&.message} : #{url}")
      if response
  ...
  def get_auth_token
    ...
    Rails.logger.debug { "EDS timing AUTH: #{Time.zone.now - s_time}s" }
