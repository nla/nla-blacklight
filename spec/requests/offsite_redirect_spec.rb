    ...
    context "when the URL is a known eResource URL" do
      ...
      context "when requested from a staff subnet" do
        ...
        it "logs access" do
          ...
          expect(Rails.logger).to have_received(:info).with(
            a_string_including("eResources staff access:", "http://opac.newsbank.com/select/shaw/35846")
          ).once
        end
      end
          ...
          context "when user is logged in" do
            ...
            context "when eResource type is 'ezproxy'" do
              ...
              it "logs access" do
                ...
                expect(Rails.logger).to have_received(:info).with(
                  a_string_including("eResources external access by user #{user.id}:", "http://www.macquariedictionary.com.au/login")
                ).once
              end
            end
          end
