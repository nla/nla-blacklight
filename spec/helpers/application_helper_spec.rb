...
RSpec.describe ApplicationHelper do
  ...
  describe "#eresources_link_url" do
    ...
    context "when user is not logged in" do
      it "returns the guest URL" do
        expect(url).to eq "https://www.nla.gov.au/eresources"
      end

      context "when ERESOURCES_GUEST_URL is configured" do
        before do
          allow(ENV).to receive(:fetch).and_call_original
          allow(ENV).to receive(:fetch).with("ERESOURCES_GUEST_URL", "https://www.nla.gov.au/eresources").and_return("https://custom-guest.example.com/eresources")
          allow(ENV).to receive(:[]).and_call_original
          allow(ENV).to receive(:[]).with("EBSCO_ERESOURCES_URL").and_return(nil)
        end

        it "returns the configured guest URL" do
    ...
    context "when user is logged in" do
      ...
      context "when EBSCO_ERESOURCES_URL is configured" do
        before do
          allow(ENV).to receive(:[]).and_call_original
          allow(ENV).to receive(:[]).with("EBSCO_ERESOURCES_URL").and_return("https://research.ebsco.com/c/ciw6tp")
        end
      ...
      context "when EBSCO_ERESOURCES_URL is not configured" do
        before do
          allow(ENV).to receive(:[]).and_call_original
          allow(ENV).to receive(:[]).with("EBSCO_ERESOURCES_URL").and_return(nil)
        end
